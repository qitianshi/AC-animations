"""Voltage and current for passive components in AC circuits."""
...
line1, = ax.plot([], [], label='V')
line2, = ax.plot([], [], label='I')
...
def _anim(i):
    ...
    y2 = np.sin(2 * np.pi * (x - 0.01 * i - 0.25 * np.pi))
    line1.set_data(x, y1)
    line2.set_data(x, y2)
    return (line1, line2)
...
anim = FuncAnimation(fig, _anim, init_func=_init, frames=100, interval=16,
                     blit=True)
anim.save('videos/ac/VI_capacitor.gif', writer='imagemagick')
