...
ax_phasor.add_patch(plt.Circle(
    (0, 0), radius=0.878, facecolor='white', edgecolor='darkgray', ls='--'))
...
parallelo1, = ax_phasor.plot([], [], c='gray', linewidth=0.5, linestyle=':')
parallelo2, = ax_phasor.plot([], [], c='gray', linewidth=0.5, linestyle=':')
...
def _init():
    ...
    parallelo1.set_data([], [])
    parallelo2.set_data([], [])
    return (line1, line2, line3, parallelo1, parallelo2)
...
def _anim(i):
    ...
    y2 = 0.5 * np.sin(2 * np.pi * (x - 0.01 * i) + 1)
    line1.set_data(x, y1)
    line2.set_data(x, y2)
    ...
    phasor2_coordinates = arrows.rotate_tail(
        0, 0, 0.5, -1 * (i / 100 - 1/(2 * np.pi)) * 360 - 90
    )
    phasor2_tail = (
    ...
    phasor_sum_coordinates = arrows.rotate_tail(
        0, 0, 0.878, -1 * (i / 100 - 1/(4 * np.pi)) * 360 - 90
    )
    phasor_sum_tail = (
        phasor_sum_coordinates[0] + phasor_sum_coordinates[2],      # x
        phasor_sum_coordinates[1] + phasor_sum_coordinates[3]       # y
    )
    ...
    parallelo1.set_data([[phasor1_tail[0], phasor_sum_tail[0]],
                         [phasor1_tail[1], phasor_sum_tail[1]]])
    parallelo2.set_data([[phasor2_tail[0], phasor_sum_tail[0]],
                         [phasor2_tail[1], phasor_sum_tail[1]]])

    return (
        line1, line2, line3,
        phasor1, horiz_connector1,
        phasor2, horiz_connector2,
        phasor_sum, horiz_connector_sum,
        parallelo1, parallelo2
    )
...
anim = FuncAnimation(fig, _anim, init_func=_init, frames=100, interval=16,
                     blit=True)
anim.save('videos/ac/add_phasorsine_samefreq_parallelo.gif',
    writer='imagemagick')
