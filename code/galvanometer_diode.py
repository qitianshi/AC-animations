"""Travelling clipped AC wave with galvanometer."""
...
# Circuit diagram
image = mpimg.imread('resources/ac_galvanometer_diode.png')
...
def _anim(i):
    ...
    y = np.clip(np.sin(2 * np.pi * (x - 0.01 * i)), 0, None)
    line.set_data(x, y)
...
anim = FuncAnimation(fig, _anim, init_func=_init, frames=100, interval=16,
                     blit=True)
anim.save('videos/ac/galvanometer_not_ac.gif', writer='imagemagick')
